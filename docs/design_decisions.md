...
**Rationale for (b)**: Python dataclass field defaults are evaluated at class-definition time (module import), capturing values once. Moving to `get_settings()` makes reads fresh each call.
